    ...
    function makeThingJSCode(exportCredentials) {
        ...
        for (i=0;i<nodes.length;i++) {
            if ('genThingJSCode' in nodes[i]._def) {
                ...
                if(type == 'subflow') {
                    next += 'let $$n_ctx={' + nns.wires.map((wire, line) => {
                        return wire.length == 0
                            ? (nns.id in subflowsManifest ? `${line}:$$ctx[${line}]` : `${line}:$$nop`)
                            : `${line}:function($$i) {` + wire.map((id) => {
                            return `$$nd["${id}"]($$cpy($$i), $$ctx);`
                        }).join('') + '}';
                    }).join(',') + '};';
                    ...
                    if(!subflow_nns.in.length || ((subflow_nns.in.length == 1) && !subflow_nns.in[0].wires.length)) {
                        ...
                        next += 'let $$n=function($$i){' + subflow_nns.in[0].wires.map((item) => {
                            return `$$nd["${item.id}"]($$cpy($$i), $$n_ctx);`
                        }).join('') + '};';
                    }
                } else {
                    if (nns.id in subflowsManifest) {
                        subflowsManifest[nns.id].map((out) => {
                            retFlow += `if($$fnd($$i[${out.port}])){$$ctx[${out.wire}]($$cpy($$i[${out.port}]));}`;
                        });
                    }
                    if (!nns.wires.length) {
                        next += 'let $$n=' + (!retFlow.length ? '$$nop;' : `function(i){${retFlow}};`);
                    } else {
                        next += 'let $$n=function($$i){';
                        next += nns.wires.map(
                            (wire, line) => wire.map(
                                (id) => `if($$fnd($$i[${line}])){$$nd["${id}"]($$cpy($$i[${line}]),$$ctx);}`
                            ).join('')
                        ).join('');
                        next += `${retFlow}};`;
                    }
                ...
                if(mjs.func) {
                    code += `,"${nns.id}": function($$i, $$_ctx){let $$ctx=$$_ctx;${next}${mjs.func}}`;
                }
                if(mjs.initialize) {
                    bootstrap += `(function(){let $$ctx={};${next}${mjs.initialize}})();`;
                }
            }
        }
        code += `};${bootstrap}`;
