...
RED.nodes = (function() {
    ...
    function makeThingJSCode(exportCredentials) {
        debugger;
        var i;
        var code = `let $$nop=function(){};let $$nd={"rev":"${RED.nodes.version()}"`;
        var bootstrap = '';
        for (i=0;i<nodes.length;i++) {
            if ('genThingJSCode' in nodes[i]._def) {
                ...
                if (!nns.wires.length) {
                    next = 'let $$n=$$nop;';
                } else if ((nns.wires.length == 1) && (nns.wires[0].length == 1)) {
                ...
                code += `,"${nns.id}": function($$i){${next}${mjs.code}}`;
                if(mjs.bootstrap) {
                    bootstrap += `$$nd["${nodes[i].id}"]();`;
                }
            }
        }
        code += `};${bootstrap}`;
