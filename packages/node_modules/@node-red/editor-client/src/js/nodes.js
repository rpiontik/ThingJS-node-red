...
RED.nodes = (function() {
    ...
    function addSubflow(sf, createNewIds) {
        ...
        RED.nodes.registerType("subflow:"+sf.id, {
            ...
            genThingJSCode: function() {
                debugger;
                var subflow = subflows[this.type.split(':')[1]];
                var nns = convertSubflow(subflow, true);
                console.info(subflow, nns);
                return {
                    func: '$$n($$i);'
                }
            },
    ...
    function makeThingJSCode(exportCredentials) {
        var i;
        var code = `let $$nop=function(){};let $$fnd=function(v){return typeof v !== "undefined"};let $$nd={"rev":"${RED.nodes.version()}"`;
        var bootstrap = '';
        for (i=0;i<nodes.length;i++) {
            if ('genThingJSCode' in nodes[i]._def) {
                ...
                if(type == 'subflow') {
                    debugger;
                    next = '$$n_ctx={' + nns.wires.map((wire, line) => {
                    ...
                    if(!subflow_nns.in.length || ((subflow_nns.in.length == 1) && !subflow_nns.in[0].wires.length)) {
                        next += 'let $$n=$$nop;';
                    } else if ((subflow_nns.in.length == 1) && (subflow_nns.in[0].wires.length == 1)) {
                        next += `let $$n=$$nd["${subflow_nns.in[0].wires[0].id}"];`;
                    } else {
                        next += 'let $$n=function($$i, $$ctx){' + subflow_nns.in[0].wires.map((item) => {
                            return `$$nd["${item.id}"]($$i, $$n_ctx || $$ctx);`
                        }).join('') + '}';
                    }
                    //var nns = convertSubflow(node, true);
                    console.info(next);
                } else {
                    if (!nns.wires.length) {
                        next = 'let $$n=$$nop;';
                    } else if ((node.outputs == 1) && (nns.wires[0].length == 1)) {
                        next = `let $$n=$$nd["${nns.wires[0][0]}"];`;
                    } else {
                ...
                if(mjs.func) {
                    code += `,"${nns.id}": function($$i, $$ctx){${next}${mjs.func}}`;
                }
                if(mjs.initialize) {
                    bootstrap += `(function(){${next}${mjs.initialize}})();`;
                }
            }
        }
        code += `};${bootstrap}`;
