    ...
    function makeThingJSCode(exportCredentials) {
        ...
        for (i=0;i<nodes.length;i++) {
            if ('genThingJSCode' in nodes[i]._def) {
                ...
                if(type == 'subflow') {
                    ...
                    if(!subflow_nns.in.length || ((subflow_nns.in.length == 1) && !subflow_nns.in[0].wires.length)) {
                        ...
                        next += '"$n":function($i){' + subflow_nns.in[0].wires.map((item) => {
                            return `$$nd["${item.id}"]($$cpy($i), this.$nc);`
                        }).join('') + '}';
                    }
                } else {
                    if (nns.id in subflowsManifest) {
                        subflowsManifest[nns.id].map((out) => {
                            retFlow += `if($$fnd($i[${out.port}])){$c["${out.wire}"]($$cpy($i[${out.port}]));}`;
                        });
                    }
                    if (!nns.wires.length) {
                        ...
                        next += nns.wires.map(
                            (wire, line) => wire.map(
                                (id) => `if($$fnd($i[${line}])){$$nd["${id}"]($$cpy($i[${line}]),this.$c);}`
                            ).join('')
                        ).join('');
                        next += `${retFlow}}`;
                    }
                ...
                if(mjs.func) {
                    code += `,"${nns.id}": function($i, $c){${next},"$e":function($i){${mjs.func}}}).$e($i);}`;
                }
                if(mjs.initialize) {
