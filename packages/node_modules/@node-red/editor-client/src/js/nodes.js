...
RED.nodes = (function() {
    ...
    function uglifyConfig() {
        // Tuning online - https://skalman.github.io/UglifyJS-online/
        return {
            parse: {
                bare_returns: false,
                ecma: 8,
    ...
    //ThingJS mJS code generator
    function makeThingJSSubflowManifest(exportCredentials) {
        var result = {};
        for (var i in subflows) {
            var subflow = convertSubflow(subflows[i], true);
            subflow.out.map((port, portIndex) => {
                port.wires.map((wire, wIndex) => {
                    console.log(subflow, port);
                    if(!result[wire.id]) {
                        result[wire.id] = [];
                    }
                    result[wire.id].push({
    ...
    function makeThingJSCode(exportCredentials) {
        //docs - http://wikihandbk.com/wiki/Node-RED:%D0%91%D0%B8%D0%B1%D0%BB%D0%B8%D0%BE%D1%82%D0%B5%D0%BA%D0%B8/node-red/Delay
        var subflowsManifest = makeThingJSSubflowManifest(exportCredentials);
        var i;
        var indexes = {
            index: 0,
            keys: {},
            getKey: function(uuid) {
                return uuid in this.keys ? this.keys[uuid] : this.keys[uuid] = ++this.index
            }
        };
        var code = `let $g={},$_mid=0;function $$nop(){}function $$fnd(v){return typeof v !== "undefined"}`
                 + `function $$cpy(s){return JSON.parse(JSON.stringify(s))}`
                 + `function $$mid(){return JSON.stringify(++$_mid)}`
                 + `let $$nd={"rev":"${RED.nodes.version()}"`;

        var bootstrap = '';
        var raw = '';
        for (i=0;i<nodes.length;i++) {
            if ('genThingJSCode' in nodes[i]._def) {
                var node = nodes[i];
                var nns = convertNode(nodes[i], exportCredentials);

                if(node.d) {
                    code += `,"${indexes.getKey(nns.id)}": $$nop`;
                    continue;
                }

                var mjs = node._def.genThingJSCode.call(nodes[i]);
                var type = node.type.split(':')[0];
                var next = '({"$c":$c,';
                var retFlow = '';

                if(type == 'subflow') {
                    next += '"$nc":{' + nns.wires.map((wire, line) => {
                        return wire.length == 0
                            ? (nns.id in subflowsManifest ? `"${line}":$c["${line}"]` : `"${line}":$$nop`)
                            : `"${line}":function($i) {` + wire.map((id) => {
                            return `$$nd["${indexes.getKey(id)}"]($$cpy($i), this.$c);`
                        }).join('') + '}';
                    }).join(',') + '},';

                    var subflow_nns = convertSubflow(subflows[node.type.split(':')[1]], true);
                    if(!subflow_nns.in.length || ((subflow_nns.in.length == 1) && !subflow_nns.in[0].wires.length)) {
                        next += '"$n":$$nop';
                    } else {
                        next += '"$n":function($i){' + subflow_nns.in[0].wires.map((item) => {
                            return `$$nd["${indexes.getKey(item.id)}"]($$cpy($i), this.$nc);`
                        }).join('') + '}';
                    }
                } else {
                    if(mjs.env) {
                        for (var key in mjs.env) {
                            next += `${JSON.stringify(key)}:${mjs.env[key]},`;
                        }
                    }
                    next += `"$id":${JSON.stringify(indexes.getKey(node.id))},`;
                    if (nns.id in subflowsManifest) {
                        subflowsManifest[nns.id].map((out) => {
                            retFlow += `$$fnd($i[${out.port}])&&this.$c["${out.wire}"]($$cpy($i[${out.port}]));`;
                        });
                    }
                    if (!nns.wires.length) {
                        next += '"$n":' + (!retFlow.length ? '$$nop' : `function($i){${retFlow}}`);
                    } else {
                        next += '"$n":function($i){';
                        next += nns.wires.map(
                            (wire, line) => wire.map(
                                (id) => `$$fnd($i[${line}])&&$$nd["${indexes.getKey(id)}"]($$cpy($i[${line}]),this.$c);`
                            ).join('')
                        ).join('');
                        next += `${retFlow}}`;
                    }
                ...
                if(mjs.func) {
                    code += `,"${indexes.getKey(nns.id)}": function($i, $c){${next},"$e":function($i){${mjs.func}}}).$e($i);}`;
                }
                if(mjs.initialize) {
                    bootstrap += `(function(){let $c={};${next},"$e":function($i){${mjs.initialize}}}).$e({})})();`;
                }
                if(mjs.raw) {
                    raw += mjs.raw;
                }
        ...
        code += `};${bootstrap}${raw}`;
        console.info('Original code: ',code);
        code = minify(code, uglifyConfig());
        console.info('Uglify code: ',code);
        return null;
    }
    ...
    return {
        ...
        makeThingJSCode: makeThingJSCode,
        id: getID,
        dirty: function(d) {
            if (d == null) {
                return dirty;
