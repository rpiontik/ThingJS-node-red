    ...
    function makeThingJSCode(exportCredentials) {
        ...
        for (i=0;i<nodes.length;i++) {
            if ('genThingJSCode' in nodes[i]._def) {
                var mjs = nodes[i]._def.genThingJSCode.call(nodes[i]);
                var nns = convertNode(nodes[i], exportCredentials);
                var next;
                if (!nns.wires.length) {
                    next = 'let $$n=$$nop;';
                } else if ((nodes[i].outputs == 1) && (nns.wires[0].length == 1)) {
                    next = `let $$n=$$nd["${nns.wires[0][0]}"];`;
                } else {
                    next = 'let $$n=function($$i){';
                    next += nns.wires.map(
                        (wire, line) => wire.map(
                            (id) => nodes[i].outputs == 1
                                ? `if($$fnd($$i)){$$nd["${id}"]($$i);}`
                ...
                if(mjs.func) {
                    code += `,"${nns.id}": function($$i){${next}${mjs.func}}`;
                }
                if(mjs.initialize) {
