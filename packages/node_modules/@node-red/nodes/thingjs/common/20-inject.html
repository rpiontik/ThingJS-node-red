...
(function() {
    ...
    RED.nodes.registerType('inject',{
            ...
            $('#node-input-property-container').css('min-height','120px').css('min-width','450px').editableList({
                addItem: function(container,i,opt) {
                    ...
                    var propertyValue = $('<input/>',{class:"node-input-prop-property-value",type:"text"})
                        ...
                        .appendTo(row)
                        .typedInput({default:'str',types:['msg','flow','str','num','bool','json','bin']});
        ...
        genThingJSCode: function() {
            var result = {
                requires: [],
                initialize: 'this.inject = function () {this.$n([{',
            };

            this.props.map((prop, index) => {
                result.initialize += (index ? ',' : '') + `${JSON.stringify(prop.p)}:`;
                var value = undefined;
                var type = prop.vt || 'str';
                switch(prop.p) {
                    case 'payload':
                        value = this.payload;
                ...
                switch (type) {
                    case 'msg':
                        ...
                        result.initialize += `$i[${field}]`;
                        break;
                    case 'num':
                        result.initialize += JSON.stringify(1 * value);
                        break;
                    case 'bool':
                        result.initialize += JSON.stringify(!!value);
                        break;
                    case 'json':
                    ...
                    case 'bin':
                        try {
                            ...
                            result.initialize += '"' + (bin && bin.length ? '\\x' : '') + bin.map((item) => {
                                return item.toString(16);
                            }).join('\\x') + '"';
                        } catch(e) {
                            result.initialize += '"' + (bin && bin.length ? '\\x' : '') + value.split("").map((item) => {
            ...
            if (this.repeat || this.once) {
                result.requires.push({
                    'timer': 'timers'
                });

                var repeatCode = this.repeat
                    ? `$$r.timer.setInterval(function(scope){scope.inject();},${1000 * this.repeat},scope || this);`
                    : ''
                ;

                if(this.once) {
                    result.initialize += `$$r.timer.setTimeout(function(scope){`
                        + `scope.inject();${repeatCode}`
                        + `},${1000 * this.onceDelay},this);`;
                } else {
                    result.initialize += repeatCode;
                }
            ...
            if(this.crontab) {
                ...
                var cronparts = this.crontab.split(" ");
                var days = cronparts[4] == "*" ? [0, 1, 2, 3, 4, 5, 6] : cronparts[4].split(",");
                var interval = 60000;
                result.initialize += `$$r.timer.setInterval(function () {`
                    + `let tm=$$r.clock.parse();`
                    + `if(`;

                if (!isNaN(cronparts[0]) && !isNaN(cronparts[1])) {
                    // Fixed time
                    result.initialize += `(tm.hour === ${1 * cronparts[1]})&&(tm.min === ${1 * cronparts[0]})`;
                } else {
                    // interval - time period
                    var minutes = cronparts[0].slice(2);
                    if (minutes === "") {
                    ...
                    interval = 1 * minutes * 60000;
                    result.initialize += `(tm.hour >= ${1 * start})&&(tm.hour <= ${1 * end})`;
                }

                result.initialize += (cronparts[4] == "*" ? '' : '&&' + (days.map((day) => `(tm.wday === ${day})`).join('&&')))
                                + `){inject();}}, ${interval});`
            }
