...
(function() {
    ...
    RED.nodes.registerType('inject',{
        ...
        genThingJSCode: function() {
            var result = {
                requires: [],
                initialize: 'let inject = function () {$$n({',
            };

            this.props.map((prop, index) => {
                result.initialize += (index ? ',' : '') + `${JSON.stringify(prop.p)}:`;
                var value = prop.p == 'payload' ? this.payload : prop.v;
                var type = (prop.p == 'payload' ? this.payloadType : prop.vt) || 'str'
                switch (type) {
                    case 'msg':
            ...
            if (this.repeat || this.once) {
                ...
                var repeatCode = this.repeat
                    ? `$$r.timer.setInterval(function () {inject();}, ${1000 * this.repeat});`
                    : 'inject();'
                ;

                if(this.once) {
                    result.initialize += `$$r.timer.setTimeout(function () {`
                        + `inject();${repeatCode}`
                        + `}, ${1000 * this.onceDelay});`;
                } else {
