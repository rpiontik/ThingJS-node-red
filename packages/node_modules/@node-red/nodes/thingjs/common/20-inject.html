    ...
    RED.nodes.registerType('inject',{
        ...
        genThingJSCode: function() {
            var result = {
                requires: [],
                bootstrap: true,
                code : 'let inject = function () {$$n({'
            };

            this.props.map((prop, index) => {
                result.code += (index ? ',' : '') + `${JSON.stringify(prop.p)}:`;
                ...
                switch (type) {
                    case 'msg':
                        ...
                        break;
                    case 'num':
                        result.code += JSON.stringify(1 * value);
                        break;
                    case 'bool':
                        result.code += JSON.stringify(!!value);
                        break;
                    case 'json':
                        result.code += value;
                        break;
                    case 'bin':
                        try {
                            ...
                            result.code += '"' + (bin && bin.length ? '\\x' : '') + bin.map((item) => {
                                return item.toString(16);
                            }).join('\\x') + '"';
                        } catch(e) {
                            result.code += '"' + (bin && bin.length ? '\\x' : '') + value.split("").map((item) => {
            ...
            if (this.repeat || this.once) {
                ...
                if(this.once) {
                    result.code += `$$r.timer.setTimeout(function () {`
                        + `inject();${repeatCode}`
                        + `}, ${1000 * this.onceDelay});`;
                } else {
                    result.code += repeatCode;
                }
            ...
            if(this.crontab) {
                ...
                var interval = 60000;
                result.code += `$$r.timer.setInterval(function () {`
                    + `let tm=$$r.clock.parse();`
                    + `if(`;

                if (!isNaN(cronparts[0]) && !isNaN(cronparts[1])) {
                    // Fixed time
                    result.code += `(tm.hour === ${1 * cronparts[1]})&&(tm.min === ${1 * cronparts[0]})`;
                } else {
                    // interval - time period
                    var minutes = cronparts[0].slice(2);
                    if (minutes === "") {
                    ...
                    result.code += `(tm.hour >= ${1 * start})&&(tm.hour <= ${1 * end})`;
                }

                result.code += (cronparts[4] == "*" ? '' : '&&' + (days.map((day) => `(tm.wday === ${day})`).join('&&')))
                                + `){inject();}}, ${interval});`
            }
