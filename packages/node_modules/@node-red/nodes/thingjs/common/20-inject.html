    ...
    RED.nodes.registerType('inject',{
        ...
        genThingJSCode: function() {
            var result = {
                requires: [],
                signature: '123',
                code : 'let proc = function () {$n({'
            };

            this.props.map((prop, index) => {
                var field = JSON.stringify(prop.p);
                result.code += (index ? ',' : '') + `${field}:`;
                switch (prop.vt || 'контекстной str') {
                    case 'msg':
                        field = JSON.stringify(prop.v);
                        result.code += `$$i[${field}]`;
                        break;
                    case 'num':
                        result.code += JSON.stringify(1 * prop.v);
                        break;
                    case 'bool':
                        result.code += JSON.stringify(!!prop.v);
                        break;
                    case 'json':
                    case 'bin':
                        var bin = JSON.parse(prop.v);
            ...
            if (this.repeat || this.once) {
                ...
                var repeatCode = this.repeat
                    ? `$$r.timer.setInterval(function () {proc();}, ${1000 * this.repeat});`
                    : 'proc();'
                ;
            ...
            if(this.crontab) {
                ...
                if (!isNaN(cronparts[0]) && !isNaN(cronparts[1])) {
                    // Fixed time
                    var time = cronparts[1]+":"+cronparts[0];
                    console.info('Fixed time at ', time, ' in ', days);
                } else {
                    // interval - time period
                    var minutes = cronparts[0].slice(2);
                    if (minutes === "") {
                    ...
                    console.info('Interval every ', minutes, ' from ', start, ' until ', end);
                }
            }
