...
//      {v:"tail",t:"switch.rules.tail",kind:'S'},
    ...
    RED.nodes.registerType('switch', {
            ...
            $("#node-input-rule-container").css('min-height','150px').css('min-width','450px').editableList({
                addItem: function(container,i,opt) {
                    ...
                    function createValueField(){
                        return $('<input/>',{class:"node-input-rule-value",type:"text",style:"margin-left: 5px;"}).appendTo(row).typedInput({default:'str',types:['msg','flow','global','str','num', previousValueType]});
                    }

                    function createNumValueField(){
                        return $('<input/>',{class:"node-input-rule-num-value",type:"text",style:"margin-left: 5px;"}).appendTo(row).typedInput({default:'num',types:['flow','global','num']});
                    }

                    function createBtwnValueField(){
                        return $('<input/>',{class:"node-input-rule-btwn-value",type:"text",style:"margin-left: 5px;"}).appendTo(row).typedInput({default:'num',types:['msg','flow','global','str','num',previousValueType]});
                    }
                    ...
                    function createBtwnValue2Field(){
                        return $('<input/>',{class:"node-input-rule-btwn-value2",type:"text",style:"margin-left:2px;"}).appendTo(row3).typedInput({default:'num',types:['msg','flow','global','str','num',previousValueType]});
                    }
                    ...
                    if ((rule.t == "btwn") || (rule.t == "index")) {
                        ...
                        typeValueField.typedInput('type','str');
        ...
        genThingJSCode: function() {
            var checkall = ((this.checkall == "true") || (this.checkall === true));
            var func = this.rules.map((rule, wire) => {
                var result = !checkall && wire > 0 ? ' else ' : '';
                ...
                switch (this.propertyType){
                    case 'msg':
                        ...
                        break;
                    default:
                        throw new Error(`Error property type for swith node ${this.propertyType}`);
                }

                var b = [['v', 'vt'], ['v2', 'v2t']].map((item) => {
                    if(item[1] in rule) {
                        switch (rule[item[1]]){
                            case 'str':
                                return JSON.stringify(rule[item[0]]);
                            case 'num':
                                return JSON.stringify(1 * rule[item[0]]);
                            default:
                                throw new Error(`Error property type for swith node [${rule[item[1]]}]`);
                        }
                    }
                });
                ...
                switch (rule.t) {
                    ...
                    case 'nnull':
                        ...
                        break;
                    case 'istype':
                        result += `if(typeof ${a}===${b[0]})`;
                        break;
                    case 'empty':
                        result += `if((${a}===unknown)||(${a}===null)||(((typeof ${a}==='string')||(typeof ${a}==='array'))&&(${a}.length===0)))`;
                        break;
                    case 'nempty':
                        result += `if((((typeof ${a}==='string')||(typeof ${a}==='array'))&&(${a}.length>0))||((${a}!==unknown)&&(${a}!==null)))`;
                        break;
                    case 'else':
                        result += 'if(els)';
                        break;
                    default:
                        throw new Error(`Unknown rule type [${rule.t}]`)
                }

                return result + `{res[${wire}]=$i;els=false;}`;
            }).join('');
            return {
                func : `let res=[];let els=true;${func}this.$n([res]);`
            }
        }
