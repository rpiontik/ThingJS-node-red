...
<script type="text/javascript">
    RED.nodes.registerType('change', {
        ...
        genThingJSCode: function() {
            ...
            var code = this.rules.map((rule) => {
                var dist = thingjsValueStringify(rule.p, rule.pt);
                switch(rule.t) {
                    case 'set':
                        return `${dist}=${thingjsValueStringify(rule.to, rule.tot)}`;
                        break;
                    case 'change':
                        result.requires.str = "string";
                        return `${dist}=$r.str.replaceAll(${dist},${thingjsValueStringify(rule.from, rule.fromt)},${thingjsValueStringify(rule.to, rule.tot)})`;
                    case 'delete':
                        return `delete ${dist}`;
                    case 'move':
                        var src = thingjsValueStringify(rule.to, rule.tot);
                        return `${dist}=${src};delete ${src}`;
                        break;
                }
            }).join(";");

            result.func = `${code};this.$n([$i]);`;
