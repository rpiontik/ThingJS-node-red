...
<script type="text/javascript">
    RED.nodes.registerType('template',{
        ...
        genThingJSCode: function() {
            ...
            var result = {
                requires: {},
                func : `${{"msg": "$i", "flow": "this.$c", "global": "$g"}[this.fieldType]}[${JSON.stringify(this.field)}]`
            }

            var output = `${JSON.stringify(this.template)}`;

            if(this.syntax == 'mustache') {
                result.requires.str = "string";
                output = `$r.str.mustache(${output},$i)`
            }
            if(this.output == "json")
                output = `JSON.parse(${output})`;

            result.func += `=${output};this.$n([$i]);`
