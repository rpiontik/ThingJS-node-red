...
<script type="text/javascript">
    RED.nodes.registerType('trigger',{
        ...
        genThingJSCode: function() {
            debugger;
            var env = {
                d: this.duration * {"ms":1, "s":1000, "min":60000, "hr":3600000}[this.units],
                rst: JSON.stringify(this.reset)
            };
            var code = `this._id='_tgr_'+this.$id` + (this.bytopic=="topic" ? `+'_'+$i.topic` : '') + `;`
                     + `this.tp=$i.topic;`;
                     + `if(($i.reset||$i.payload===this.rst)&&$g[this._id]){clearTimeout($g[this._id]);$g[this._id]=undefined;return}`;
            if (this.duration == "0") { //block: wait to be reset
                console.info('block: wait to be reset');
            } else if ((this.duration * 1) < 0) { //loop: wait for
                console.info('loop: wait for');
            } else { //wait: resend it every
                switch(this.op1type) {
                    case 'nul':
                        break;
                    case 'pay':
                        code += `!$g[this._id]&&this.$n([$i]);`
                        break;
                    default:
                        env.op1 = thingjsValueStringify(this.op1, this.op1type);
                        code += `!$g[this._id]&&this.$n([{"payload":this.op1,"topic":this.tp}]);`
                }
                switch(this.op2type) {
                    case 'nul':
                        break;
                    case 'pay':
                        ...
                        break;
                    case 'payl':
                        code += 'this.op2=$$cpy($i.payload);';
                        break;
                    default:
                        env.op2 = thingjsValueStringify(this.op2, this.op2type);
                }

                if(this.op2type!='nul') {
                    code += `!$g[this._id]&&($g[this._id]=$r.timer.setTimeout(function(c){c.$n([{"payload":c.op2,"topic":c.tp}]);$g[c._id]=null},this.d,this))`;
                }
            }
