            ...
            switch(this.mode || "auto") {
                case "auto":
                    ...
                    code  += `this.s=function($i){`
                          ...
                          +     `delete $i.parts;this.$n([$i]);acc=null`
                          + `};`
                ...
                case "custom":
                    ...
                    var push_code = `acc.p.push($i[this.prop]||"");acc.c++;acc.i=$i;`;
                    switch (this.build) {
                        case `array`:
                        ...
                        case `merged`:
                            push_code = `acc.i=$i;acc.pl=acc.pl||{};`
                                      + `if($i.payload)for(let k in $i.payload){acc.pl[k]=$i.payload[k];acc.c++}`;
                    ...
                    code += `let acc=$g[this.jid]||{"p":[],"c":0};`
                        ...
                        +     `${apply_code}`
                        +     `if(!this.accum)acc=null;this.$n([$i])`
                        +  `}`
                        + `$g[this.jid]=acc;`
                    ;
