...
<script type="text/javascript">
    RED.nodes.registerType('split',{
        ...
        genThingJSCode: function() {
            ...
            var make_msg = `this.mki=function($i,pl,t,k,i,c){$i.payload=pl;$i.parts={"id":$$mid(),"type":t};`
                         + (
                            this.stream
                            ? `if(t==="string"){$i.parts.index=$g["_spt_${this.id}"]++}else{$i.parts.index=i;$i.parts.key=k;$i.parts.count=c}`
                            : `$i.parts.index=i;$i.parts.key=k;$i.parts.count=c;`
                         )
                         + `return $i};`;
            return {
                raw: this.stream ? `$g["_spt_${this.id}"]=0;`: undefined,
                requires: {
                    "str" : "string"
                },
                func : `${make_msg}`
                    ...
                    + `else if((tp==="array")||("length" in $i.payload)){tp="array";${is_array}}`
